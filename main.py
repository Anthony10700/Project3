...
import gamepy as GAP
import macgyver as MAG
...
def main(game_inst, display):
    ...
    if not game_inst.get_reload:  # get reload bool for look if game is restart
        if CON.NB_SPRITE_X == 15:  # if number of sprite in X is 15 read in file a map lvl
    ...
    needle.objet_caractere = CON.CHAR_OF_NEEDLE  # set special char for map list
    ...
    ether.objet_caractere = CON.CHAR_OF_ETHER  # set special char for map list
    ...
    plastic_tube.objet_caractere = CON.CHAR_OF_PLASTIC_TUBE  # set special char
    game_inst.set_map_game(plastic_tube.set_position_random(plastic_tube.map))
    # set random position

    macgyver_obj = MAG.MacGyver(1)  # macgyver object from class
    ...
    while gameloop:  # loop on the game
        for event in display.my_pygame.event.get():  # loop for look event on keyboard
            if event.type == display.my_pygame.QUIT:   # if the mouse click on x screen
                gameloop = False  # set gameloop to 0 for quit the loop
            ...
            if event.type == display.my_pygame.KEYDOWN:  # event keydown press
                ...
                if show_is_ok:
                    game_inst.set_map_game(macgyver_obj.get_map)  # get map list to MAG
                    display.set_map_list(game_inst.get_map_game)  # set map game to display
                    ...
                    if tmp == CON.CONSTANT_WIN:  # if end of game is win
                    ...
                    elif tmp == CON.CONSTANT_LOSE:  # if end of game is lose
...
if __name__ == "__main__":
    Mygame = GAP.GamePy()  # create instance gamepy

    Mydisplay = DIS.Display(Mygame.read_map_in_file(),
                          Mygame.get_id_map,
                          Mygame.get_lvl_map,
